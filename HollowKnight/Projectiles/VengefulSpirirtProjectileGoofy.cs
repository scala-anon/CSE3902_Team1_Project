...
{
    public class VengefulSpiritProjectileGoofy : Projectile
    {
        ...
        public VengefulSpiritProjectileGoofy(Vector2 position, Vector2 velocity, ProjectileFaction faction)
            : base(position, velocity, faction)
        {
            _facing = velocity.X >= 0 ? Direction.Right : Direction.Left;
            _currentSprite = SpriteFactory.Instance.CreateNickPainterProjectile(position);
